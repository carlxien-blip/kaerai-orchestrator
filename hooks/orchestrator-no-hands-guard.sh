...
msg="[orchestrator has no hands] You're acting directly on a spoke from inside the hub session: ${hit}. **Self-classify first** — [keeping truth / cross-project coordination] (editing a canonical pointer, asset registry, brain-hands architecture) -> fine, continue; [doing work] (producing content / running analysis / pulling data / writing an implementation) -> **stop**: dispatch a worker (first line of the brief MUST force onboarding: 'first read all of <spoke>/CLAUDE.md and walk its skill index / canonical pointers, then act'), or run it inside the spoke's own session. **Being able to act is not the same as it being your job to act.** This warning does not block."
